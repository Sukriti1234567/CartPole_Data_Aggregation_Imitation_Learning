...
There is both a simple Behavior Cloning model and a Dataset Aggregation (DAgger) model implemented in the model. The Dataset Aggregation is able to produce more accurate results, as, unlike the Behavior Cloning model, the Dataset Aggregation takes what the expert model would do as another piece of data to help it create a decision.
...
However, the Dataset Aggregation model was not completely accurate all the time:

![Learning Curve](data_aggregation_learning_curve.png)

To run this code, clone the repository. Change file paths in the files. Download dependencies: pip install gymnasium torch numpy matplotlib stable-baselines3. Run collect_expert_data.py first, then cloning_training.py. Then, run dataset_aggregation.py. Then finally run evaluate.py and evaluate_bc.py.
